...
def C2I():
	...
	for file in files:
		...
		if(len(tmp) == 1):
			...
			os.system('riscv32-unknown-elf-gcc -march=rv32gc -mabi=ilp32 -E -o ' './file_i/' + tmp[0] + '.i ' + './file_c/' + file)
...
def I2S():
	...
	for file in files:
		...
		if(len(tmp) == 1):
			...
			os.system('riscv32-unknown-elf-gcc -march=rv32gc -mabi=ilp32 -S -o ' + './file_s/' + tmp[0] + '.s ' + path_files_i + '/' + file)
...
def S2O():
	...
	for file in files:
		...
		if(len(tmp) == 1):
			...
			os.system('riscv32-unknown-elf-gcc -march=rv32gc -mabi=ilp32 -c -o ' + './file_o/' + tmp[0] + '.o ' + path_files_s  + '/' + file)
